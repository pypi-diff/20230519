# Comparing `tmp/curvit-1.6.6.tar.gz` & `tmp/curvit-1.6.7.tar.gz`
...
## Comparing `curvit-1.6.6.tar` & `curvit-1.6.7.tar`
...
### Comparing `curvit-1.6.6/LICENSE` & `curvit-1.6.7/LICENSE`

 * *Files identical despite different names*

### Comparing `curvit-1.6.6/PKG-INFO` & `curvit-1.6.7/PKG-INFO`
...
### Comparing `curvit-1.6.6/README.md` & `curvit-1.6.7/README.md`

 * *Files identical despite different names*

### Comparing `curvit-1.6.6/curvit/check_curvit_variability_V.0.4.py` & `curvit-1.6.7/curvit/check_curvit_variability_V.0.4.py`

 * *Files identical despite different names*

### Comparing `curvit-1.6.6/curvit/curvit.py` & `curvit-1.6.7/curvit/curvit.py`
...
+radius_pixels = np.array(
+    [1.5, 2, 2.5, 3, 4, 5, 7, 9, 12, 15, 20, 30, 40, 50, 70, 80, 95]
+)
+
+nuv_energy_percentage = np.array(
+    [
+        29.9,
+        42.0,
+        52.0,
...
+)
+
+fuv_energy_percentage = np.array(
+    [
+        28.1,
+        40.7,
+        51.1,
...
+
+# ratio = measured CPF / actual CPF
+nuv_ratio = nuv_energy_percentage / 100.0
+fuv_ratio = fuv_energy_percentage / 100.0
+fuv_ratio_function = interp1d(radius_pixels, fuv_ratio, kind="cubic")
+nuv_ratio_function = interp1d(radius_pixels, nuv_ratio, kind="cubic")
...
+def weighted_centre(x_coords=None, y_coords=None, weights=None, bins=600):
+    x_array, x_bin_edges = np.histogram(x_coords, bins=bins, weights=weights)
+    y_array, y_bin_edges = np.histogram(y_coords, bins=bins, weights=weights)
...
+    mask = (x_mesh - image_x_centre) ** 2 + (
+        y_mesh - image_y_centre
+    ) ** 2 <= mask_radius**2
+    array = np.array([flat_counts, x_mesh, y_mesh]).T
...
+    pos_x, pos_y, sub_size, cir_rad, sub_name, fx, fy, path_to_events_list, events_list
+):
+    mask = np.logical_and(
+        np.abs(fx - pos_x) <= sub_size, np.abs(fy - pos_y) <= sub_size
+    )
...
+    mean, _, std = sigma_clipped_stats(data, sigma=5.0, maxiters=1)
+    # to avoid std becoming zero.
     if std == 0:
         mean = np.mean(data)
         std = np.std(data)
...
+        uA.append(A[0])
+        A = np.array(
+            [x for x in A if x not in A[KDTree(A).query_ball_point(uA[-1], 16)]]
+        )
...
+    RCORR = ICORR * (0.89 - (0.30 * (ICORR**2)))
+    RCORR_err = RCORR * np.sqrt(
+        (ICORR_err**2) + ((0.30 * 2 * ICORR * ICORR_err) ** 2)
+    )
...
-                                                 radius, 
-                                                 aperture_correction)
+        CPF, CPF_err = apply_aperture_correction(
+            CPF, CPF_err, radius, aperture_correction
+        )
...
-                                 range = (time_start, till_here))
+        counts, _ = np.histogram(T, bins=nbin, range=(time_start, till_here))
...
-                                              weights = W)    
+    weighted_counts, bin_edges = np.histogram(T, bins=ranges, weights=W)
 
-    counts, _ = np.histogram(T, bins = ranges)
+    counts, _ = np.histogram(T, bins=ranges)
 
-    bin_centres = (starts + ends) / 2.
+    bin_centres = (starts + ends) / 2.0
...
+def make_image(events_list=events_list, framecount_per_sec=framecount_per_sec):
+    """Create a FITS image from the input events list.
+    The image unit is in counts.
...
+
+# Remove soon.
+def makefits(events_list=events_list, ramecount_per_sec=framecount_per_sec):
+    print("Deprecated function. Please use make_image instead.")
+    return make_image(events_list=events_list, framecount_per_sec=framecount_per_sec)
+
+
...
-    uA = np.array([sources['xcentroid'].data, sources['ycentroid'].data]).T
-    return uA 
+    bkg_MAD = median_abs_deviation(for_bkg_binned_data, axis=None, nan_policy="omit")
+
+    daofind = DAOStarFinder(fwhm=6, threshold=threshold * bkg_MAD, exclude_border=True)
+
+    sources = daofind(zoomed_data - bkg_median)
+    sources.sort("mag")
+    uA = np.array([sources["xcentroid"].data, sources["ycentroid"].data]).T
...
+            print(
+                "\n{} ignored.\nExposure time below {} seconds.".format(
+                    path, min_exptime
+                )
+            )
             continue
...
-    mask = photons > 0 
+    hdu_base = fits.open(combined_eventslist_name, mode="update")
+    hdu_base[0].header["AVGFRMRT"] = (
+        AVGFRMRT,
+        "Added by events lists combine software",
+    )
+    hdu_base[0].header["STDFRMRT"] = (
+        STDFRMRT,
+        "Added by events lists combine software",
+    )
+    hdu_base[0].header["RA_PNT"] = RA_pointing
...
### Comparing `curvit-1.6.6/curvit.egg-info/PKG-INFO` & `curvit-1.6.7/curvit.egg-info/PKG-INFO`
...
### Comparing `curvit-1.6.6/setup.py` & `curvit-1.6.7/setup.py`
